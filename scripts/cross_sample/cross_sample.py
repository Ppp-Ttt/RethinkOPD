"""
Cross-sampling generation with student/teacher routing on JS divergence.

For each token position:
  1. student and teacher each produce next-token logits
  2. decide_sampler(...) — picks "student" or "teacher" based on a JS check
...
def make_run_tag() -> str:
    ...
    return f"cross_sample_{stu}_TCH_{tch}_jsth{JS_THRESHOLD}_topk{ROUTER_TOP_K}"
...
def js_divergence_on_union_batched(
    logits_stu: torch.Tensor,
    logits_tch: torch.Tensor,
    top_k: int = ROUTER_TOP_K,
    js_temperature: float = JS_TEMPERATURE,
) -> torch.Tensor:
    """
    Batched JS divergence on UNION top-k support.

    Args:
        logits_stu, logits_tch: (B, V) float tensors on the same device.

    Returns:
        js: (B,) float32 tensor on the same device. Per row, the union support
    ...
    """
    z_s = logits_stu.float() / js_temperature
    z_t = logits_tch.float() / js_temperature
    B, V = z_s.shape
    ...
    kl_s = (p_s * (log_ps - log_m)).sum(dim=-1)
    kl_t = (p_t * (log_pt - log_m)).sum(dim=-1)
    return 0.5 * (kl_s + kl_t)                                   # (B,)
...
def decide_sampler_batched(
    ...
    *,
    ...
    Returns:
        ...
        js_values:   (B,) float32 tensor — per-row JS values.
    """
    js = js_divergence_on_union_batched(logits_stu, logits_tch, top_k=top_k,
                                        js_temperature=js_temperature)
    use_teacher = js > js_threshold
    return use_teacher, js
...
def cross_sample_batch(
    ...
    *,
    ...
    Strategy:
      ...
      - per-token state (response ids, router decisions, JS values) is kept in
    ...
    with torch.inference_mode():
        ...
        js_buf        = torch.zeros((B, max_new_tokens),
        ...
        for step in range(max_new_tokens):
            # Routing.
            use_teacher, js_b = decide_sampler_batched(last_logits_s, last_logits_t)
        ...
        with open(tmp_path, "w", encoding="utf-8") as fout:
            ...
            for batch_start in range(0, len(work_units), BATCH_SIZE):
                ...
                for (sample_idx, seed), result in zip(batch_units, results):
                    ...
                    mean_js = (sum(result["js_values"]) / n_total) if n_total > 0 else 0.0
...
def run_generation(enable_thinking: bool):
    print(f"\n{'='*70}")
    print(f"  Cross-sampling generation")
    print(f"  student  = {STUDENT_MODEL}")
    print(f"  teacher  = {TEACHER_MODEL}")
    print(f"  routing  = JS>{JS_THRESHOLD} (union top-{ROUTER_TOP_K}, T={JS_TEMPERATURE})"
