"""
Plot the distribution of student-side intersection probability mass over
...
Edit the global parameters below, then run:
  python plot_student_intervention_mass_distribution.py
"""
...
OUTPUT_PATH = (
    "/mmu_cd_ssd/pengtiantian/projects/OPD/results/cross_sample/cross_sample_Qwen3-1.7B-Base_TCH_Qwen3-4B-Base-GRPO_js/cross_sample_Qwen3-1.7B-Base_TCH_Qwen3-4B-Base-GRPO_jsth0.06_topk16/amc23_t0.7_n8-MNT7168_top16_student_intervention_mass_distribution.png"
)
...
def main():
    ...
    for key, ent in entropy_records.items():
        ...
        s_mass = np.asarray(iou["student_intersection_mass"], dtype=np.float32)
        iou_decisions = iou["router_decisions"]

        L = len(decisions)
        if len(s_ent) != L or len(t_ent) != L or len(s_mass) != L:
            length_mismatches += 1
            continue
    ...
    print(f"Student mass mean/median: {masses.mean():.4f} / {np.median(masses):.4f}")
    print(f"Student mass min/max   : {masses.min():.4f} / {masses.max():.4f}")
    ...
    ax.set_xlabel("Student-side intersection probability mass")
    ax.set_ylabel("Share of selected positions")
    ax.set_title(
        f"Student intersection mass distribution\n"
        f"(teacher intervention & student entropy > {S_ENT_THR} "
        f"& teacher entropy < {T_ENT_THR})"
    )
