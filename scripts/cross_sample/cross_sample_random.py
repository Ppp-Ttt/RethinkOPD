"""
Cross-sampling generation with RANDOM student/teacher routing.

Baseline for the JS / KL routed variants: the routing signal carries no
information about the two distributions at all — at every token position the
teacher is used with probability RANDOM_PROB (env var), independently per
position and per sequence.

For each token position:
  1. student and teacher each produce next-token logits
  2. a Bernoulli(RANDOM_PROB) draw picks "teacher" or "student"
...
and avg@N is reported, together with the *realised* fraction of tokens that
were routed to the teacher (mean_teacher_ratio) — which should track
RANDOM_PROB closely.
...
# Overridable via env var RANDOM_PROB
# (e.g. `RANDOM_PROB=0.1 python cross_sample_random.py`).
RANDOM_PROB = float(os.environ.get("RANDOM_PROB", "0.1"))  # P(teacher) per token
...
def make_run_tag() -> str:
    ...
    return f"cross_sample_{stu}_TCH_{tch}_randp{RANDOM_PROB}"
...
def decide_sampler_batched(
    ...
    device: torch.device,
    *,
    random_prob: float = RANDOM_PROB,
    generator: torch.Generator | None = None,
) -> torch.Tensor:
    """
    Batched random routing decision.

    Returns:
        use_teacher: (B,) bool tensor on `device` — True for teacher route,
        drawn i.i.d. Bernoulli(random_prob).
    """
...
def cross_sample_batch(
    ...
    *,
    ...
    """
    Batched per-token cross-sampling with random routing. Generates
    len(prompt_ids_list) responses in parallel on a single GPU.
    ...
    Strategy:
      ...
      - per-token state (response ids, router decisions) is kept in
      ...
      - both the routing draw and the sampling draw use `generator`
    ...
    with torch.inference_mode():
        ...
        for step in range(max_new_tokens):
            # Routing.
            use_teacher = decide_sampler_batched(B, device,
...
def run_generation(enable_thinking: bool):
    print(f"\n{'='*70}")
    print(f"  Cross-sampling generation (RANDOM routing)")
    print(f"  student  = {STUDENT_MODEL}")
    print(f"  teacher  = {TEACHER_MODEL}")
    print(f"  routing  = Bernoulli(P(teacher)={RANDOM_PROB}) per token")
...
def grade_file(file_path: Path) -> dict:
    ...
    for ex_id in sorted(records.keys()):
        ...
        for it in items:
            ...
            length = int(it.get("response_length", 0))
            response_lengths.append(length)
            n_teacher_tokens += round(float(it.get("teacher_ratio", 0.0)) * length)
            n_all_tokens += length
        per_problem.append({
            "example_id": ex_id,
            "n": len(items),
    ...
    summary = {
        ...
        # Token-weighted realised teacher fraction over the whole file.
        "overall_teacher_ratio": (n_teacher_tokens / n_all_tokens) if n_all_tokens else 0.0,
    ...
    print(f"  target P(teacher)  : {RANDOM_PROB:.4f}")
    print(f"  mean teacher_ratio : {summary['mean_teacher_ratio']:.4f}")
    print(f"  overall teacher_ratio (token-weighted): {summary['overall_teacher_ratio']:.4f}")
...
def main():
    ...
    if not args.skip_gen:
        print("\n" + "=" * 70)
        print("  STEP 1 / 2 : Cross-sampling generation (RANDOM routing)")
        print("=" * 70)
