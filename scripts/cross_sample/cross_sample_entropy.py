"""
Cross-sampling generation with student/teacher routing on one model's entropy.

For each token position:
  1. student and teacher each produce next-token logits
  2. decide_sampler(...) — picks "student" or "teacher" based on the entropy
     of the configured model's next-token distribution
...
# --- Models ---
STUDENT_MODEL = "/mmu_cd_ssd/pengtiantian/projects/OPD/models/Qwen3-4B-Base"
TEACHER_MODEL = "/mmu_cd_ssd/pengtiantian/projects/OPD/checkpoint/Qwen3-8B-Base-GRPO_DAPO-Math-17k-Processed_Qwen3-8B-Base_Qwen3-4B_7168-T_1.0-Tch_1.0-n_8-mbs_64-topk_0-topk_strategy_union-rw_student_p-2026-06-16_15-22-00/global_step_279/Qwen3-8B-Base-GRPO_step279"

# --- Routing (cross-sampling) ---
# Overridable via env var ENTROPY_THRESHOLD (e.g. `ENTROPY_THRESHOLD=0.5 python cross_sample_entropy.py`).
# Falls back to the default below when unset, so the script still runs directly.
ENTROPY_THRESHOLD = float(os.environ.get("ENTROPY_THRESHOLD", "0.5"))  # entropy > threshold → teacher; else student
ENTROPY_TEMPERATURE = 1.0   # entropy computed at this temperature
ENTROPY_SOURCE = os.environ.get("ENTROPY_SOURCE", "student").lower()
if ENTROPY_SOURCE not in {"student", "teacher"}:
    raise ValueError("ENTROPY_SOURCE must be 'student' or 'teacher'")
...
def make_run_tag() -> str:
    ...
    entropy_tag = "enth" if ENTROPY_SOURCE == "student" else "tchenth"
    return f"cross_sample_{stu}_TCH_{tch}_{entropy_tag}{ENTROPY_THRESHOLD}"
...
def entropy_batched(
    logits: torch.Tensor,
    entropy_temperature: float = ENTROPY_TEMPERATURE,
) -> torch.Tensor:
    """
    Batched entropy of a model's next-token distribution.

    Args:
        logits: (B, V) float tensor.

    Returns:
        ent: (B,) float32 tensor on the same device — per-row entropy in nats.
    """
    z = logits.float() / entropy_temperature
    log_probs = F.log_softmax(z, dim=-1)
    probs = log_probs.exp()
    # H(p) = -sum p * log p  (positions with p=0 contribute 0 via 0*log0 := 0)
...
def decide_sampler_batched(
    ...
    *,
    ...
    """
    Batched routing decision based on the configured model's entropy.
    ...
    Returns:
        ...
        ent_values:  (B,) float32 tensor — per-row entropy in nats.
    """
    if entropy_source == "student":
        entropy_logits = logits_stu
    elif entropy_source == "teacher":
        entropy_logits = logits_tch
    else:
        raise ValueError("entropy_source must be 'student' or 'teacher'")
    ent = entropy_batched(entropy_logits, entropy_temperature=entropy_temperature)
    use_teacher = ent > entropy_threshold
    return use_teacher, ent
...
def cross_sample_batch(
    ...
    *,
    ...
    Strategy:
      ...
      - per-token state (response ids, router decisions, entropy values) is
        kept in preallocated GPU tensors of shape (B, max_new_tokens) — no
        per-step CPU sync
    ...
    with torch.inference_mode():
        ...
        ent_buf       = torch.zeros((B, max_new_tokens),
        ...
        for step in range(max_new_tokens):
            # Routing.
            use_teacher, ent_b = decide_sampler_batched(last_logits_s, last_logits_t)
        ...
        with open(tmp_path, "w", encoding="utf-8") as fout:
            ...
            for batch_start in range(0, len(work_units), BATCH_SIZE):
                ...
                for (sample_idx, seed), result in zip(batch_units, results):
                    ...
                    mean_ent = (sum(result["entropy_values"]) / n_total) if n_total > 0 else 0.0
...
def run_generation(enable_thinking: bool):
    print(f"\n{'='*70}")
    print("  Cross-sampling generation (entropy routing)")
    print(f"  student  = {STUDENT_MODEL}")
    print(f"  teacher  = {TEACHER_MODEL}")
    print(f"  routing  = H({ENTROPY_SOURCE})>{ENTROPY_THRESHOLD} (T={ENTROPY_TEMPERATURE})"
          f" → teacher else student")
...
def main():
    ...
    if not args.skip_gen:
        print("\n" + "=" * 70)
        print("  STEP 1 / 2 : Cross-sampling generation (entropy routing)")
        print("=" * 70)
